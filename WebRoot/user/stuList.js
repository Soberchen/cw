layui.extend({
    ...
    var tableIns = table.render({
        elem: '#newsList',
        url : 'StudentServlet?action=selectStudent',
        toolbar: '#toolbarDemo',
        page : false,//开启分页
        height: 'full-145',
        ...
        cols : [[
            ...
            {field: 'uname', title: '姓名', minWidth:100, align:"center",sort:true},
            {field: 'className', title: '班级',  align:'center',sort:true},  
            {field: 'teacher', title: '教员', minWidth:100, align:"center",sort:true},   
            {field: 'leader', title: '班主任', minWidth:100, align:"center",sort:true},
        ]]
    });
    /*------------- 加载用户数据 --end------------------------------*/
    ...
    $("#doSubmit").click(function(){
    	var like = $("#likename").val()
    	 ...
    	 tableIns.reload({
	      url:"http://localhost:8080/ClockClass/StudentServlet?action=selectStudent&uname="+like,
	      page: {
	        curr: 1 //重新从第 1 页开始
	      }
	    });
    ...
    function upStudent(id){
    	layui.layer.open({
    		title : "修改用户信息",
    		type : 2,
    		content : "admin/page/system/student/stuInfo.jsp",
    		area:['400px','540px'],
    		success:function(layero, index){
    			$.ajax({
    				url:"/ClockClass/StudentServlet?action=selectStudentid",
    				type:"post",
    				data:{"id":id},
    				success:function(data){
    					...
    					body.find("#className").val(info.className);
    					body.find("#teacher").val(info.teacher);
    					body.find("#leader").val(info.leader);
    					/*//性别(单选)		
    					var sex2 = info.sex;
    					if(sex2 == 0){
   ...
   //删除用户
    function delStudent(id){
    	$.ajax({
    		url:"StudentServlet?action=delStudent",
    		data:{"id":id},
    		type:"post",
    		success:function(data){
    			if(data == 1){
    ...
    function addStudent(){
    	layui.layer.open({
    		title : "添加学员",
    		type : 2,
    		content : "admin/page/system/student/stuAdd.jsp",
    		area:['400px','490px'],
    	})
    }
