...
(function(b, c) {
	function a(d) {
		this.opts = {
			...
			"dom" : this.Q(d.dom),
			"time" : d.time || "",
			"maskClose" : (d.maskClose + "").toString() !== "false" ? true
					: false,
			"callback" : d.callback || ""
		};
		this.rnd = this.rnd();
	...
	a.prototype = {
		...
		isMobile : function() {
			return navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i) ? true
					: false
		},
		...
		events : function() {
			...
			d.addEvent(d.mask, (d.isMobile() ? "touchend" : "click"), function(
					f) {
				if (d.opts.maskClose) {
					d.close()
				}
